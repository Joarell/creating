import * as clean from "./layer.puzzle.man";
import * as arrange from "./labor.same.sizes.man";


//This function provides de map of each sizes found at howManySizes.
export function defineCrate(works_sizes) {
	let sizes;
	let crate;
	let i;
	let result;
...
export function zeroSizes(work_list, sizes) {
	let crate;
	let tmp;
	let len;
	let dump;
...
export function howManySizes(works) {
	let len;
	let counter;
	let i;
	let last;
...
//of the list.
export function checking(arr, works, length) {
	let cleaner;
...
export * from "./same.sizes.checker"
